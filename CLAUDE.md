...
| **Telegram Bot (`TelegramBotService`)** | **Done — observer + operator stop: push notifications (drawdown halt, trade executed via Redis `trade_executed` channel), `/status`/`/portfolio`/`/signals`/`/help` query commands; `/stop` cancels all orders, closes all positions, exits loop (sends confirmation when cleanup completes); pipeline runs fully automatically with no human gate; disabled when token/chat_id unset** |
