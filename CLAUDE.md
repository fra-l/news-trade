...
| **Telegram Bot (`TelegramBotService`)** | **Done — read-only observer: push notifications (drawdown halt, trade executed via Redis `trade_executed` channel), `/status`/`/portfolio`/`/signals`/`/help` query commands; pipeline runs fully automatically with no human gate; disabled when token/chat_id unset** |
