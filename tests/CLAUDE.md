...
| `test_telegram_bot.py` | `TelegramBotService` — `request_approval()` (approve/block/timeout/fail-open paths), command handlers (`/halt`, `/resume`, `/status`, `/portfolio`, `/signals`), access control (wrong chat_id dropped), callback handler (future resolution), `stop()` (pending futures resolved, app shutdown) | `MagicMock` Telegram `Application` + `AsyncMock` bot; in-memory SQLite |
